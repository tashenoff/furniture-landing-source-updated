  ...
  return (
    <section id="certificates" className="py-20 bg-gray-50">
      <div className="container mx-auto px-4">
        <div className="mb-16">
          <h2 className="text-4xl font-bold text-center mb-4 relative z-10">Гарантии и сертификаты</h2>
        </div>
        ...
        <div ref={ref} className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {certificates.map((certificate, index) => (
            <div
              key={index}
              className="premium-block transform transition-all duration-500"
              style={{
                opacity: inView ? 1 : 0,
            ...
            >
              <div className="flex flex-col items-start">
                <div className="w-16 h-16 flex items-center justify-center rounded-full bg-black text-white mb-6 relative overflow-hidden group">
                  <FontAwesomeIcon icon={certificate.icon} className="text-2xl relative z-10 transition-transform duration-500 group-hover:scale-110" />
                </div>
                <h3 className="text-xl font-bold mb-4 text-left">{certificate.title}</h3>
                <p className="text-gray-600 text-left">{certificate.description}</p>
              </div>
            </div>
          ))}
        </div>
